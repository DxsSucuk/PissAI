package de.presti.pissai.discord.bot;

import de.presti.pissai.discord.bot.version.*;
...
public class BotWorker {
     ...
     */
    public static void createBot(String botToken, BotVersion version1, String build1) throws LoginException {
        version = version1;
        token = botToken;
        state = BotState.INIT;
        build = build1;

        shardManager = DefaultShardManagerBuilder.createDefault(token).setShardsTotal(1).enableIntents(GatewayIntent.getIntents(GatewayIntent.ALL_INTENTS)).disableIntents(GatewayIntent.GUILD_PRESENCES).setMemberCachePolicy(MemberCachePolicy.ALL).disableCache(CacheFlag.EMOJI, CacheFlag.ACTIVITY).build();
    }
